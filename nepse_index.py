...
def get_nepse_index(url: str = NEPSE_URL, force_refresh: bool = False) -> float | None:
    ...
    if not force_refresh and _last_fetch_monotonic and elapsed < MIN_REQUEST_INTERVAL_SECONDS:
        return _last_index
...
def get_header_text() -> str:
    """Convenience helper to fetch current index and return header text."""
    return render_header(get_nepse_index())
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch and print NEPSE header text.")
    parser.add_argument(
        "--watch",
        action="store_true",
        help="Continuously refresh the header output.",
    )
    parser.add_argument(
        "--interval",
        type=int,
        default=5,
        help="Refresh interval in seconds when using --watch (must be 5-10).",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Bypass local 5-second cache for a one-off fetch.",
    )
    return parser
...
if __name__ == "__main__":
    ...
    if args.watch:
        run_header_loop(interval_seconds=args.interval)
    else:
        print(render_header(get_nepse_index(force_refresh=args.force)))
